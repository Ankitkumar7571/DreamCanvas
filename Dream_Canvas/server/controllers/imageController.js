...
export const generateImage = async (req, res) => {
  ...
  if (!process.env.CLIPDROP_API) {
    console.error("❌ ClipDrop API key not configured (CLIPDROP_API)");
    return res.status(500).json({ success: false, message: "ClipDrop API key not configured. Set CLIPDROP_API in your .env." });
  }
  try {
    ...
    console.error("❌ ClipDrop Error:", {
      status,
      message: parsed.message || parsed.error || decoded || error.message,
    });

    res.status(status).json({
      success: false,
      status,
      message: parsed.message || parsed.error || decoded || error.message || "Unknown error",
    });
  }
};
